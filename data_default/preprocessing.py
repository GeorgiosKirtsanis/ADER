...
def read_data(dataset_path):
    ...
    if filename_extension == 'dat':
        sess_map, item_map, reformed_data = util.read_dat(dataset_path)
    elif filename_extension == 'csv':
        sess_map, item_map, reformed_data = util.read_csv(dataset_path)
    else:
...
def short_remove(reformed_data, args):
    ...
    for [userId, _, time] in reformed_data:
        org_sess_end = util.generate_sess_end_map(org_sess_end, userId, time)
    ...
    for [userId, _, time] in removed_data:
        sess_end = util.generate_sess_end_map(sess_end, userId, time)
    ...
    if args.is_time_fraction:
        ...
        for period in sorted(time_fraction.keys()):
            for i, [userId, itemId, time] in enumerate(time_fraction[period]):
                itemId = util.generate_name_Id_map(itemId, item_map)
