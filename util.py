...
class Evaluator:
    ...
    def evaluate(self,
        ...
        return self.display(epoch, is_print)

    def results(self) -> (float, float, float, float):
        """ This method returns evaluation results (MRR@5, RECALL@5, NDCG@5, ACCURACY).
        """
        valid_user = len(self.ranks)
        valid_ranks_5 = list(filter(lambda x: x < 5, self.ranks))
        RECALL_5 = len(valid_ranks_5)
        MRR_5 = sum(map(lambda x: 1.0 / (x + 1), valid_ranks_5))
        NDCG_5 = sum(map(lambda x: 1.0 / math.log(x + 2, 2), valid_ranks_5))
        ACCURACY = len(list(filter(lambda x: x < 1, self.ranks)))

        return MRR_5 / valid_user, RECALL_5 / valid_user, NDCG_5 / valid_user, ACCURACY / valid_user

    def display(self,
                epoch,
                is_print) -> str:
        """ This method display and save evaluation metrics (MRR@5, RECALL@5, NDCG@5, ACCURACY).
            Returns:
                info (str): Evaluation results information.
        """
        results = self.results()
        info = 'epoch:%d, %s (MRR@5: %.4f, RECALL@5: %.4f, NDCG@5: %.4f, ACCURACY: %.4f)' \
